package com.webapps.app.security;
...
public class JwtTokenUtil implements Serializable {
	...
	//generate token for user with Google Account
	public String generateTokenWithGoogleAccounts(String googleId) {
				Map<String, Object> claims = new HashMap<>();
				return doGenerateToken(claims, googleId);
	}
	...
	//validate facebook or google token
		public Boolean validateFaceBookOrGoogleToken(String token, Users user) {
			final String username = getUsernameFromToken(token);
			if(username.equals(user.getFacebookId()) && !isTokenExpired(token)) {
				return true;
			}else if(username.equals(user.getGoogleId()) && !isTokenExpired(token)) {
				return true;
			} else {
				return false;
			}
