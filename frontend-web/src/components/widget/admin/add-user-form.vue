...
<v-container>
    <v-card>
        ...
        <v-card-title>
            <v-select :items="filterDropDownList" v-model="dataFilter" label="Role" outlined item-text="name" item-value="id" color="rgb(0, 209, 178)" @change="filterData"></v-select>
        ...
        <v-data-table class="table-container" :headers="tableHeaderList" :items="dataTableList" :search="search">
            ...
            <template v-slot:item.action="{ item }">
                <v-icon small class="mr-2" @click="editListener(item)">
                    edit
                </v-icon>
                <v-icon small @click="deleteListener(item)">
                    delete
                </v-icon>
            </template>
        </v-data-table>
            ...
            <v-container>
                <div ref="formContainer" class="form-container">
                    ...
                    <v-form ref="form" v-model="valid" width="300">
                        <v-text-field v-model="data.id" label="Id" required outline flat color="rgb(0, 209, 178)" class="hidden"></v-text-field>
                        <v-text-field v-model="data.username" :rules="useranameRules" label="Username" required outline flat color="rgb(0, 209, 178)"></v-text-field>
                        <v-text-field v-model="data.email" :rules="emailRules" label="Email" required outline flat color="rgb(0, 209, 178)"></v-text-field>
                        <v-text-field v-model="data.password" :rules="passwordRules" :type="show1 ? 'text' : 'password'" @click:append="show1 = !show1" :append-icon="show1 ? 'visibility' : 'visibility_off'" label="Password" hint="At least 8 characters" required outline flat color="rgb(0, 209, 178)"></v-text-field>

                        <v-select :items="dataDropdownList" v-model="data.roles" label="Role" outlined item-text="name" return-object color="rgb(0, 209, 178)"></v-select>
