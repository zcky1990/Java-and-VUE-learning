      ...
      <v-flex xs12 d-flex>
        <div class="refs">
          ...
          <div class="reference-container">
            <div
              v-for="(item, index) in data.reference_list"
              ref="refereceContainer"
              class="ref-container"
              :key="item"
            >
              <v-text-field v-model="data.reference_list[index]" label="Reference" :value="item">
...
export default {
  ...
  methods: {
    ...
    validBtn: function() {
      if (
        this.data.article_content.length > 0 &&
        this.data.article_title.length > 0 &&
        this.data.categoryArticle.length > 0
      ) {
